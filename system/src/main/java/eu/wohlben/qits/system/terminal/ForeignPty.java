 ...
 *
 * <p><b>COPIED VERBATIM from qits-projects</b> ({@code control/ForeignPty}), which paid for every
 * line of it. Only the package and the two sentences naming that service's own use case differ.
 * Change the code here and change it there; the reachability metadata beside it is part of the
 * copy.
 *
 * <p><b>Why this exists rather than pty4j.</b> A terminal needs a real TTY: {@code glances} draws
 * with cursor addressing and reads its keys from the tty, and a shell run under {@code docker exec
 * -t} only gets a job-control-capable session when the CLI itself has a controlling terminal. A
 * pipe gets a program that either refuses to run or renders nothing. pty4j gave us a TTY, at the
 * price of JNA plus
 ...
 * the bad one — the JVM suite stays green and the binary throws on the first terminal. Six libc
 * entry points reached directly cost the builder nothing and remove the dependency outright.
 *
 ...
 * {@code META-INF/native-image/eu.wohlben.qits/qits-platform-system-domain/reachability-metadata.json},
 * and the two have to move together: change a signature here and the binary either fails to build
 * or throws {@code MissingForeignRegistrationError} on the first terminal, while the JVM suite goes
 ...
 * this kernel's. This service runs on the platform host, beside the docker daemon it reads, and
 * that host is Linux; a different platform should get its own implementation rather than a
...
final class ForeignPty implements AutoCloseable {
   ...
   *     spawn, which reaches the operator as a 503
   */
  static ForeignPty open(int cols, int rows) throws IOException {
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      // concurrent terminal opens would race.
   ...
   * resize racing the child's exit is ordinary and must not surface as an error to the browser.
   */
   ...
   * window does, and what ends a docker CLI that is still attached to a container.
   */
  @Override
  public void close() {
