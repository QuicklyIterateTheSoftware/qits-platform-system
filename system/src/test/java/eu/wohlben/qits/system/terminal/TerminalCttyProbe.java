 ...
 * or not the bug was present. {@link TerminalCttyTest} launches this class under {@code setsid},
 ...
 * TerminalProcesses#terminalProcess} does, and reports two things: whether <em>this</em> process
 * ended up owning the terminal, and how many SIGHUPs it was sent when the master was closed. Both
 * must be zero.
 */
public final class TerminalCttyProbe {

  private TerminalCttyProbe() {}
  ...
  public static void main(String[] args) throws Exception {
    ...
    // `sleep` stands in for the docker CLI: what matters is which process opens the slave.
    Process child =
        TerminalProcesses.terminalProcess(pty.slavePath(), java.util.List.of("sleep", "30")).start();
    Thread.sleep(500);
  ...
  /**
   * The terminal the child ended up on. It must be non-zero: a program only reaches /dev/tty when
