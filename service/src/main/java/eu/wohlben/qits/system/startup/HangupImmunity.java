 ...
 *       three times in qits-projects, and {@code TerminalProcesses.terminalProcess} — copied here
 *       with the fix intact — carries the story. This service runs MORE terminals than that one
 *       did, so the backstop travels with it.
 * </ul>
 *
 * <p>So the honest handling is to refuse it. Between "a signal nobody legitimately sends us" and "a
 * graceful shutdown that takes every live terminal with it", ignoring is the safe default, and the
